...
- Confirmed: ${(txData || []).filter(t => t.status === 'CONFIRMED').length}
- Draft: ${(txData || []).filter(t => t.status === 'DRAFT').length}
- Submissions: ${JSON.stringify(subData || [])}
- Top categories: ${JSON.stringify(Object.entries((txData || []).reduce((acc: any, t) => { acc[t.category] = (acc[t.category] || 0) + Number(t.amount_gross); return acc; }, {})).sort((a: any, b: any) => b[1] - a[1]).slice(0, 5))}
    `.trim());
    setContextLoaded(true);

    const picked: string[] = [];
    if ((txData || []).filter(t => t.status === 'DRAFT').length > 0) picked.push('How much tax should I set aside this quarter?');
