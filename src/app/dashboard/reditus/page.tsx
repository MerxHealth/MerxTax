...
function getDeadline(q: string, taxYear: string): string {
  ...
  const map: Record<string, string> = {
    Q1: `7 Aug ${y}`, Q2: `7 Nov ${y}`, Q3: `7 Feb ${y + 1}`, Q4: `7 May ${y + 1}`,
  };
  return map[q] ?? '';
}

function daysLeft(q: string, taxYear: string): number {
  const dl = new Date(getDeadline(q, taxYear));
  ...
  return (
    <span style={{ position: 'relative', display: 'inline-block', marginLeft: 4 }}>
      <span onMouseEnter={() => setV(true)} onMouseLeave={() => setV(false)}
        style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 15, height: 15, borderRadius: '50%', background: '#E8F8F2', color: '#01D98D', fontSize: 9, fontWeight: 700, cursor: 'help', border: '1px solid #01D98D' }}>
        i
      </span>
      {v && (
...
export default function ReditusPage() {
  ...
  async function handleSave() {
    ...
    const row = {
      ...
      quarter: getHMRCQuarter(dateObj), status: form.status,
  ...
  const navTab = (active: boolean): React.CSSProperties => ({
    ...
    background: active ? '#fff' : 'transparent', color: active ? '#0A2E1E' : '#6B7280',
        ...
        {screen === 'ledger' && (
          <>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 20 }}>
              ...
              <div style={summaryCard(netProfit >= 0 ? '#0A2E1E' : '#EF4444')}>
              ...
              {!loading && filtered.map(t => (
                <div key={t.id} className="tx-row" onClick={() => openEdit(t)}
                  ...
                  <span style={{ fontWeight: 700, color: t.type === 'INCOME' ? '#01D98D' : '#EF4444' }}>
                    {t.type === 'INCOME' ? '+' : '-'}{fmt(Number(t.amount_gross))}
                  </span>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              {quarterData.map(q => (
                <div key={q.quarter} style={{ background: '#fff', borderRadius: 14, border: `2px solid ${q.quarter === currentQuarter && filterTaxYear === currentTaxYear ? '#01D98D' : '#E5E7EB'}`, padding: 20 }}>
                  ...
                  <button style={{ width: '100%', padding: '8px', fontSize: 12, fontWeight: 600, borderRadius: 8, border: '1px solid #E5E7EB', background: '#F9FAFB', color: '#9CA3AF', cursor: 'not-allowed', fontFamily: "'DM Sans', sans-serif" }}>
                    Submit to HMRC - QUARTUS (coming soon)
                  </button>
                </div>
              ))}
            </div>
          ...
          <div style={{ background: '#fff', borderRadius: '20px 20px 0 0', width: '100%', maxWidth: 560, maxHeight: '90vh', overflow: 'auto', padding: '24px 24px 40px', boxShadow: '0 -4px 40px rgba(0,0,0,0.15)' }}>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <input type="number" step="0.01" min="0" placeholder="0.00" value={form.amount_gross} onChange={e => setForm(f => ({ ...f, amount_gross: e.target.value }))}
                style={{ width: '100%', padding: '10px 14px', fontSize: 24, fontWeight: 700, fontFamily: "'Montserrat', sans-serif", border: '1px solid #E5E7EB', borderRadius: 10, background: '#fff', color: '#1C1C1E', boxSizing: 'border-box', outline: 'none' }} />
            </div>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <input type="date" value={form.date} onChange={e => setForm(f => ({ ...f, date: e.target.value }))}
                style={{ width: '100%', padding: '10px 14px', fontSize: 14, border: '1px solid #E5E7EB', borderRadius: 10, background: '#fff', fontFamily: "'DM Sans', sans-serif", color: '#1C1C1E', boxSizing: 'border-box' }} />
            </div>
            ...
            <div style={{ marginBottom: 16 }}>
              <button onClick={() => setShowVAT(!showVAT)}
                ...
                {showVAT ? 'v' : '>'} VAT <Tooltip text="Only needed if you are VAT registered. Most sole traders under 90,000 turnover are not required to register." />
              </button>
              {showVAT && (
                <div style={{ marginTop: 10 }}>
                  <select value={form.vat_rate} onChange={e => setForm(f => ({ ...f, vat_rate: e.target.value }))}
                    style={{ width: '100%', padding: '10px 14px', fontSize: 14, border: '1px solid #E5E7EB', borderRadius: 10, background: '#fff', fontFamily: "'DM Sans', sans-serif", boxSizing: 'border-box' }}>
                  ...
                  {(form.vat_rate === 'STANDARD' || form.vat_rate === 'REDUCED') && form.amount_gross && (
                    <div style={{ marginTop: 8, fontSize: 12, color: '#6B7280', background: '#F9FAFB', padding: '8px 12px', borderRadius: 8 }}>
                      {form.vat_rate === 'STANDARD'
                        ? `Net: ${fmt(parseFloat(form.amount_gross)/1.2)} VAT: ${fmt(parseFloat(form.amount_gross) - parseFloat(form.amount_gross)/1.2)}`
                        : `Net: ${fmt(parseFloat(form.amount_gross)/1.05)} VAT: ${fmt(parseFloat(form.amount_gross) - parseFloat(form.amount_gross)/1.05)}`}
                    </div>
                  )}
                </div>
              )}
